...
#define MASK_BITS ~(0x0f) //1s in upper 4 bits
...
void lcd_init(void)
{
    //PortB, bit 0 and 1 -> output
    //PortD, bits 4-7 -> output
    DDRB |= 0x3;                // Set the DDR register bits for ports B and D
    DDRD |= ~(0x0f);            // Take care not to affect any unnecessary bits
...
void lcd_writenibble(unsigned char lcdbits)
{
    ...
    PORTD &= ~MASK_BITS; //clear upper 4 bits
    PORTD |= (lcdbits & MASK_BITS); //copy upper 4 bits

    /* Make E signal (PB1) go to 1 and back to 0 */
    PORTB |= 0x2;
    PORTB |= 0x2; //on for 250 ns
    PORTB &= ~(0x2);
} 
